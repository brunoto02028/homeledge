...
import { Loader2, AlertCircle, Home, Building2, Eye, EyeOff, Zap, Sparkles, Crown, ShieldCheck, CheckCircle, UserPlus } from 'lucide-react';
import { useTranslation } from '@/lib/i18n';

export default function RegisterPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin h-8 w-8 border-2 border-amber-500 border-t-transparent rounded-full" /></div>}>
      <RegisterPageInner />
    </Suspense>
  );
}
...
function RegisterPageInner() {
  ...
  const [formData, setFormData] = useState({
    ...
    selectedPlan: isInviteFlow ? '' : 'starter',
  });
  ...
  useEffect(() => {
    fetch('/api/auth/providers').then(r => r.json()).then(p => {
      if (p?.google) setGoogleAvailable(true);
    }).catch(() => {});
  }, []);
    ...
    try {
      const res = await fetch('/api/signup', {
        ...
        body: JSON.stringify({
          ...
          accountType: isInviteFlow ? 'household' : formData.accountType,
          selectedPlan: isInviteFlow ? '' : formData.selectedPlan,
          isInviteFlow,
        }),
      });
      ...
      if (data.requiresVerification) {
        router.replace(`/verify-email?email=${encodeURIComponent(data.email || formData.email)}${cbParam}`);
      } else {
      ...
      <Card className="w-full max-w-2xl">
            ...
            {isInviteFlow && (
              <div className="p-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30">
                <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400">
                  ...
                  <span className="text-sm font-medium">You&apos;ve been invited to join a team</span>
                </div>
                <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">Create your account to accept the invitation. No plan required.</p>
              </div>
            )}
        ...
        <CardFooter className="justify-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href={callbackUrl ? `/login?callbackUrl=${encodeURIComponent(callbackUrl)}` : '/login'} className="text-primary hover:underline font-medium">
              {t('register.signIn')}
            </Link>
          </p>
        </CardFooter>
