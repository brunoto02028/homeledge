  ...
  async function saveBlogPost(post: GeneratedPost, status: string) {
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        actions: [{ label: '✏´©Å Editar artigo', href: `/admin/marketing/blog/${data.id}/edit` }],
      }]);
    } catch (e: any) {
                    ...
                    {msg.actions.map((action, ai) => (
                      action.href ? (
                        <Link key={ai} href={action.href}>
                          <Button size="sm" variant={action.variant || 'outline'} className="h-7 text-xs">
                            {action.label}
                          </Button>
                        </Link>
                      ) : (
                        <Button key={ai} size="sm" variant={action.variant || 'outline'} className="h-7 text-xs" onClick={action.onClick}>
                          {action.label}
                        </Button>
                      )
                    ))}
