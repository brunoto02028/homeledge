...
async function main() {
  ...
  const entries = [
    // === API KEYS ===
    {
      ...
      notes: 'Gemini 2.0 Flash — PRIMARY AI provider for Clarity & Co. Used in lib/ai-client.ts',
    ...
    // === COMPANIES HOUSE OAUTH ===
    {
      ...
      notes: 'OAuth Client ID (username) + Client Secret (password). For electronic filing: address changes, officer appointments, confirmation statements. Redirect URI: https://clarityco.co.uk/api/government/callback/companies-house',
    ...
    // === OPEN BANKING ===
    {
      ...
      notes: 'Open Banking provider for bank sync. Client ID (username) + Client Secret (password). Redirect: https://clarityco.co.uk/api/open-banking/callback. Production mode (not sandbox).',
    ...
    // === VPS / DATABASE ===
    {
      ...
      notes: 'SSH: ssh root@5.182.18.148. Clarity & Co runs on port 3100 (PM2: homeledger). Docling on port 3200 (PM2: docling). Nginx reverse proxy to clarityco.co.uk.',
    ...
    // === DOMAIN ===
    {
      title: 'Clarity & Co Domain & SSL',
      category: 'other',
      websiteUrl: 'https://clarityco.co.uk',
      notes: 'Domain: clarityco.co.uk. SSL via Cloudflare/Nginx. DNS managed via domain registrar. App URL: https://clarityco.co.uk (port 3100 behind Nginx).',
